...
<html>
    ...
    <body>
        <div class="back">
            <span><a href="Secretary_Account.php"><button class="b">Back</button></a></span>
        </div>
        <?php
            ...
            if (isset($_POST['update'])){
                if (isset($_POST['curpass'])){
                    ...
                    if ($newpass != $conpass){
                        ...
                        $sql = "SELECT id FROM dapitan_secretary WHERE password='$curpass'";
                        $result = $conn->query($sql);

                        if (mysqli_num_rows($result)>0){
                            //$ambot = "SELECT id FROM dapitan_secretary WHERE id = '$user_id'";
                            $sql = "UPDATE dapitan_secretary SET password = '$newpass', fullname = '$fullname', username = '$username', email = '$email', phone_number = '$phone_number' WHERE id = '$user_id'";
